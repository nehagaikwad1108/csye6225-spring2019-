...
var1=$(aws cloudformation describe-stacks --stack-name "$1-IAM" --query "Stacks[0].StackId" --output text 2>&1)
...
if [ $? -eq 0 ]
        then
            echo "Stack $1-IAM successfully deleted!!!"
   else
	...
	echo "Stack $1-IAM doesn't exist"
	exit 0
fi
